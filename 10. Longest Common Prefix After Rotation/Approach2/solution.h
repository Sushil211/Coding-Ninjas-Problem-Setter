...
vector<int> kmpPreProcess(string a, int m){
    ...
    while(i < m) {
        ...
        //If the ith index of "a" doesn't match with the jth index of "a" and j > 0, then j redirects to lps[j-1].
        else if (j > 0) {
            j = lps[j-1];
        }
...
int minimumRequiredShifts(string a, string b){
    ...
    while(i < n && j < m) {
        ...
        //If the above characters do not match and j > 0, then j redirects to lps[j-1] and continue in the loop. 
        else if(j > 0) {
            j = lps[j - 1];
            continue;
        } 
        ...
        //If the value of j exceeds the max variable, then update the max to j and ans to i-j. 
        if(j > max){
        	max = j;
          	ans = i-j;
        }
